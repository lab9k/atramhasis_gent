...
from atramhasis_gent.skos.providers import GentCSVProvider
...
def parse_argv_for_import(argv):
    ...
    parser = argparse.ArgumentParser(
        ...
        usage='{0} [--from path_input_file] [--to conn_string] [--conceptscheme_label cs_label] '
              ...
              '(example: "{1} --from atramhasis/scripts/my_file --to sqlite:///atramhasis.sqlite --conceptscheme_label '
              'Labels --conceptscheme_uri urn:x-skosprovider:trees"'
              ' --uri_pattern urn:x-skosprovider:trees:%s)'.format(
            cmd, cmd)
    )
